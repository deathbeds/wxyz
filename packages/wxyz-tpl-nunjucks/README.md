...
```bash
mamba install -c conda-forge wxyz_tpl_jinja
```

or...

```bash
conda install -c conda-forge wxyz_tpl_jinja
```
...
## Reusing `@deathbeds/wxyz-tpl-nunjucks`
...
[project]
dependencies = [
    "wxyz_tpl_jinja",                     # but probably pinned sensibly
]
```
...
```yaml
{ 'devDependencies': {
      '@deathbeds/wxyz-tpl-nunjucks': '*', # but probably pinned sensibly
    }, 'jupyterlab': { 'sharedPackages': { '@jupyter-widgets/base': { 'bundled': false, 'singleton': true }, '@jupyter-widgets/controls': { 'bundled': false, 'singleton': true }, '@deathbeds/wxyz-tpl-nunjucks': { 'bundled': false, 'singleton': true } } } }
```
