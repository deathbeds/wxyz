...
The `@deathbeds/wxyz-json-schema-form` TypeScript API is documented on [ReadTheDocs][docs]. The contents of these docs are also available in-line in your editor, via the
`@deathbeds/wxyz-json-schema-form` on `npmjs.org` with:
...
## Reusing `@deathbeds/wxyz-json-schema-form`
...
[project]
dependencies = [
    "wxyz_json_schema_form",                     # but probably pinned sensibly
]
```
...
{
  "devDependencies": {
    "@deathbeds/wxyz-json-schema-form": "*"               # but probably pinned sensibly
  },
  "jupyterlab": {
    "sharedPackages": {
