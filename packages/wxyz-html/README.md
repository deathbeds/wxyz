...
**If you just want to _use_ `@deathbeds/wxyz-html` in JupyterLab 3**
...
```bash
mamba install -c conda-forge wxyz_html
```

or...

```bash
conda install -c conda-forge wxyz_html
```
...
```ts
import wxyz from '@deathbeds/wxyz-html';

console.log(wxyz); // and see _something_
```

## Reusing `@deathbeds/wxyz-html`
...
[project]
dependencies = [
    "wxyz_html",                     # but probably pinned sensibly
]
```
...
```yaml
{ 'devDependencies': {
      '@deathbeds/wxyz-html': '*', # but probably pinned sensibly
    }, 'jupyterlab': { 'sharedPackages': { '@jupyter-widgets/base': { 'bundled': false, 'singleton': true }, '@jupyter-widgets/controls': { 'bundled': false, 'singleton': true }, '@deathbeds/wxyz-html': { 'bundled': false, 'singleton': true } } } }
```
