...
**If you just want to _use_ `@deathbeds/wxyz-svg` in JupyterLab 3**
...
```bash
mamba install -c conda-forge wxyz_svg
```

or...

```bash
conda install -c conda-forge wxyz_svg
```
...
```ts
import wxyz from '@deathbeds/wxyz-svg';

console.log(wxyz); // and see _something_
```

## Reusing `@deathbeds/wxyz-svg`
...
[project]
dependencies = [
    "wxyz_svg",                     # but probably pinned sensibly
]
```
...
```yaml
{ 'devDependencies': {
      '@deathbeds/wxyz-svg': '*', # but probably pinned sensibly
    }, 'jupyterlab': { 'sharedPackages': { '@jupyter-widgets/base': { 'bundled': false, 'singleton': true }, '@jupyter-widgets/controls': { 'bundled': false, 'singleton': true }, '@deathbeds/wxyz-svg': { 'bundled': false, 'singleton': true } } } }
```
