...
`@deathbeds/wxyz-jsonld` is distributed on `npmjs.org` with:
...
```bash
jlpm add @deathbeds/wxyz-jsonld
```
...
[project]
dependencies = [
    "wxyz_jsonld",                     # but probably pinned sensibly
]
```
