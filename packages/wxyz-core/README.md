...
```bash
mamba install -c conda-forge wxyz_core
```

or...

```bash
conda install -c conda-forge wxyz_core
```

## Developer Installation

`@deathbeds/wxyz-core` is distributed on `npmjs.org` with:
...
[project]
dependencies = [
    "wxyz_core",                     # but probably pinned sensibly
]
```
